...
import { removeoverlay } from "./data/defaults.json";
import { getSceneConfiguration, getStepConfiguration } from "./functions";

declare const BattleTransition: any;

test("UI Test", async ({ page }) => {
  await openStepConfiguration(page, "removeoverlay");
  const stepItem = page.locator(`div.tab[data-tab="battle-transitions"] div.step-config-item[data-transition-type="removeoverlay"]`);
