...
export class FlashStep extends TransitionStep<FlashConfiguration> {
  ...
  static getListDescription(config?: FlashConfiguration): string {
    if (config) return _loc("BATTLETRANSITIONS.FLASH.LABEL", { duration: config.duration, background: config.backgroundType === "image" ? config.backgroundImage : config.backgroundType === "color" ? config.backgroundColor : "overlay" }) ?? "";
    else return "";
  }
